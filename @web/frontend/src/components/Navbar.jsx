  ...
  return (
    <>
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        ...
        <div className="container mx-auto px-6 xl:px-8 flex items-center justify-between">
          {/* Logo */}
            ...
            <div className="flex items-center space-x-4">
              {!isAuthenticated ? (
                <>
                  <Link 
                    to="/login" 
                    className="text-sm font-medium px-5 py-2.5 rounded-lg transition-colors duration-200 hover:bg-gray-100"
                  >
                    Log in
                  </Link>
                  <Link 
                    to="/register" 
                    className="text-sm font-medium px-5 py-2.5 rounded-lg bg-indigo-600 text-white transition-all duration-200 hover:bg-indigo-700 hover:shadow-lg"
                  >
                    Sign up
                  </Link>
                </>
                ...
                <div className="flex items-center space-x-4">
                  <div className="relative group">
                    ...
                    <div className="absolute right-0 mt-3 w-48 bg-white rounded-lg shadow-lg py-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 transform origin-top-right scale-95 group-hover:scale-100">
                      <button
          ...
          <div className="container mx-auto px-6 py-5 space-y-2">
            ...
            {!isAuthenticated ? (
              <div className="grid grid-cols-2 gap-3 mt-4 pt-4 border-t border-gray-100">
                <Link 
                  to="/login" 
                  className="py-2.5 px-4 text-center text-sm font-medium rounded-lg border border-gray-200 text-gray-700 hover:bg-gray-50"
                >
                  Log in
                </Link>
                <Link 
                  to="/register" 
                  className="py-2.5 px-4 text-center text-sm font-medium rounded-lg bg-indigo-600 text-white hover:bg-indigo-700"
                >
                  Sign up
                </Link>
              </div>
              ...
              <div className="mt-4 pt-4 border-t border-gray-100">
                ...
                <button
                  onClick={handleLogout}
                  className="w-full mt-3 py-3 px-4 text-left text-sm font-medium rounded-lg text-red-600 hover:bg-red-50"
                >
                  Log out
                </button>
              </div>
      ...
      <div className={`w-full ${scrolled ? 'h-16' : 'h-20'}`}></div>
    </>
  );
};
