...
impl AppState {
    ...
    pub fn launch_fullscreen_mode(&mut self, ctx: &egui::Context) -> Result<()> {
        log::info!("Launching fullscreen upscaling mode");
        
        // Check if we have an active profile
        let profile = match self.profile {
            Some(p) => p,
            None => {
                log::error!("Cannot launch fullscreen mode without an active profile");
                self.show_error("No Active Profile", "Please create and select a profile before upscaling.");
                return Ok(());
            }
        };
        
        // Determine capture target based on current state
        let capture_target = if let Some(hwnd) = self.selected_window {
            // Use the window the user has selected
            log::info!("Using selected window: {:?}", hwnd);
            CaptureTarget::Window(hwnd)
        } else if let Some(region) = self.region_selection {
            // Use the region the user has selected
            log::info!("Using selected region: {:?}", region);
            CaptureTarget::Region(region)
        } else {
            // Default to desktop capture if no window/region selected
            log::info!("No window or region selected, defaulting to desktop capture");
            CaptureTarget::Display(0)
        };
        
        // Display a status message
        self.show_notice("Initializing Upscaling", "Preparing to capture and upscale...");
        
        // Clear any existing frames
        self.upscaler.clear_frames();
        
        // Get upscaling technology from profile
        let upscaling_tech = match profile.upscaling_tech {
            crate::profiles::UpscalingTech::Fsr => UpscalingTech::Fsr,
            crate::profiles::UpscalingTech::Nearest => UpscalingTech::Nearest,
            crate::profiles::UpscalingTech::Bilinear => UpscalingTech::Bilinear,
            crate::profiles::UpscalingTech::Bicubic => UpscalingTech::Bicubic,
            crate::profiles::UpscalingTech::Lanczos => UpscalingTech::Lanczos,
        };
        
        log::info!("Using upscaling technology: {:?}", upscaling_tech);
        
        // Get quality setting from profile
        let quality = match profile.quality {
            crate::profiles::Quality::Performance => UpscalingQuality::Performance,
            crate::profiles::Quality::Balanced => UpscalingQuality::Balanced,
            crate::profiles::Quality::Quality => UpscalingQuality::Quality,
            crate::profiles::Quality::UltraQuality => UpscalingQuality::UltraQuality,
        };
        
        log::info!("Using quality setting: {:?}", quality);
        
        // Get algorithm from profile
        let algorithm = match profile.algorithm {
            crate::profiles::FsrAlgorithm::Easu => FsrAlgorithm::Easu,
            crate::profiles::FsrAlgorithm::Rcas => FsrAlgorithm::Rcas,
            crate::profiles::FsrAlgorithm::EasuRcas => FsrAlgorithm::EasuRcas,
        };
        ...
        let temp_status = Arc::new(Mutex::new(None));
        
        log::info!("Setting up capture thread for target: {:?}", capture_target);
