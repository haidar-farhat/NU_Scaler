...
impl From<AdapterInfo> for GpuInfo {
    fn from(info: AdapterInfo) -> Self {
        ...
        println!(
            "[GPU Detector] Found GPU: {} (Vendor ID: 0x{:X}, Device ID: 0x{:X})",
            info.name, info.vendor, info.device
        );
        println!(
            "[GPU Detector] Device type: {:?}, Backend: {:?}",
...
impl GpuDetector {
    ...
    fn determine_primary_gpu(&self) -> Option<GpuInfo> {
        ...
        let discrete_gpus: Vec<_> = self.all_gpus.iter().filter(|gpu| gpu.is_discrete).collect();
    ...
    pub fn determine_best_upscaling_technology(&self) -> UpscalingTechnology {
        match self.primary_gpu {
            Some(ref gpu) => match gpu.vendor {
                GpuVendor::Nvidia => UpscalingTechnology::DLSS,
                GpuVendor::Amd => {
                    #[cfg(feature = "fsr3")]
                    {
                ...
                _ => UpscalingTechnology::Wgpu,
            },
            None => UpscalingTechnology::Fallback,
        }
    }
    ...
    pub fn get_gpu_description(&self) -> String {
        match &self.primary_gpu {
            Some(gpu) => {
                ...
                format!(
                    "{} {} GPU: {} ({:?})",
                    gpu_type, vendor, gpu.name, gpu.backend
                )
            }
...
mod tests {
    ...
    fn test_gpu_vendor_from_id() {
        assert_eq!(
            GpuVendor::Nvidia,
            GpuInfo::from(mock_adapter_info(0x10DE)).vendor
        );
        assert_eq!(
            GpuVendor::Amd,
            GpuInfo::from(mock_adapter_info(0x1002)).vendor
        );
        assert_eq!(
            GpuVendor::Intel,
            GpuInfo::from(mock_adapter_info(0x8086)).vendor
        );
        assert_eq!(
            GpuVendor::Other,
            GpuInfo::from(mock_adapter_info(0xABCD)).vendor
        );
    }
}
